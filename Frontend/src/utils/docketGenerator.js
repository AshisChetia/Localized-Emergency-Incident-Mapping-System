...
export const generateDocketPDF = async (report) => {
  ...
  doc.setFont(fonts.heading, 'bold');
  doc.setFontSize(13);
  doc.text('Official Incident Record', 18, y + 4);
  ...
  doc.text(`Generated: ${new Date().toLocaleString('en-IN')} | Official Incident Record`, 105, pageHeight - 14, { align: 'center' });

  // Save PDF
  const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
  const fileName = `LEIMS-Incident-Record-${timestamp}.pdf`;
  try {
    doc.save(fileName);
  } catch (saveError) {
