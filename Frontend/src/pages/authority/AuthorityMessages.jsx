// ─────────────────────────────────────────
// pages/authority/AuthorityMessages.jsx
// Messaging interface for authorities to handle user queries
// ─────────────────────────────────────────

import { useState, useEffect, useRef } from "react";
import { useAuth } from "../../context/AuthContext";
import { getReportsByPincode } from "../../services/reportService";
import { getMessagesByReportId, sendMessage } from "../../services/messageService";
import Loader from "../../components/Loader";
import toast from "react-hot-toast";
import { colors, fonts } from "../../styles/designTokens";
import { 
  MessageSquare, Send, AlertCircle, FileText, Activity, Clock, ShieldCheck
} from "lucide-react";
        ...
        <div className="w-full md:w-1/3 bg-white rounded-3xl border border-[var(--c-borderLight)] shadow-sm flex flex-col overflow-hidden h-full">
          <div className="p-6 border-b border-[var(--c-borderLight)] shrink-0">
            <h2 className="text-xl font-bold text-[var(--c-charcoal)] flex items-center gap-2" style={{ fontFamily: fonts.heading }}>
              <ShieldCheck className="w-6 h-6 text-[var(--c-accentGold)]" /> Citizen Messages
            </h2>
            <p className="text-sm text-[var(--c-textSecondary)] mt-1">Select a report to respond to citizen's queries.</p>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-3 flex flex-col gap-2">
            {loadingReports ? (
              ...
              <div className="text-center p-8 text-[var(--c-textSecondary)]">
                ...
                <p className="text-sm">No reports in your jurisdiction.</p>
              </div>
            ) : (
              reports.map(report => (
                <button
             ...
             <>
              ...
              <div className="px-6 py-5 border-b border-[var(--c-borderLight)] flex items-center justify-between shrink-0 bg-[var(--c-offWhite)]/50">
                <div>
                  ...
                  <p className="text-xs font-semibold text-[var(--c-textSecondary)] mt-0.5">
                    User: {selectedReport.reporter_name || "Citizen"}
                  </p>
                </div>
              </div>
              ...
              <div className={`flex-1 overflow-y-auto p-6 flex flex-col gap-4 relative transition-opacity duration-300 ${loadingMessages ? "opacity-60" : "opacity-100"}`}>
                ...
                {messages.length === 0 && !loadingMessages ? (
                  ...
                  messages.map((msg, idx) => {
                    const isAuthority = msg.sender_type === "authority";
                    return (
                      <div key={msg.id || idx} className={`flex flex-col max-w-[80%] ${isAuthority ? "self-end items-end" : "self-start items-start"}`}>
                        <span className="text-[10px] font-bold text-[var(--c-textSecondary)] mb-1 px-1">
                          {isAuthority ? "You" : msg.sender_name}
                        </span>
              ...
              <form onSubmit={handleSendMessage} className="p-4 bg-white border-t border-[var(--c-borderLight)] shrink-0 flex gap-3 items-end">
                <textarea
                  ...
                  className="flex-1 max-h-32 min-h-[50px] bg-[var(--c-offWhite)] border border-[var(--c-borderLight)] rounded-2xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-[var(--c-accentGold)]/40 focus:border-[var(--c-accentGold)]/50 resize-none"
                  rows={2}
                  onKeyDown={(e) => {
                ...
                <button
                  ...
                  className="w-12 h-12 rounded-2xl bg-[var(--c-accentGold)] text-white flex justify-center items-center shrink-0 hover:bg-[#D4B670] disabled:opacity-50 disabled:cursor-not-allowed shadow-md transition-all active:scale-95 border border-[#F2DCA2]"
                >
