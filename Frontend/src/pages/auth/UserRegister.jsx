...
import { MapPin, Mail, Lock, Eye, EyeOff, User, Hash, Phone, ArrowRight } from "lucide-react";
...
const UserRegister = () => {
  ...
  const [formData, setFormData] = useState({ 
    name: "", email: "", password: "", confirmPassword: "", pincode: "", number: "" 
  });
  ...
  const inputClass = "w-full bg-white border border-[var(--c-borderLight)] rounded-xl pl-10 pr-3 py-2.5 text-sm focus:outline-none focus:border-[var(--c-olive)] focus:ring-1 focus:ring-[var(--c-olive)] transition-all";
  ...
  return (
    <div style={authStyle} className="min-h-[calc(100vh-80px)] flex items-center justify-center p-4 relative overflow-hidden bg-[var(--c-offWhite)]">
      ...
      <div className="relative z-10 w-full max-w-[500px] bg-white/80 backdrop-blur-xl border border-[var(--c-borderLight)] rounded-3xl shadow-2xl p-8 flex flex-col gap-5">
        ...
        <div className="flex flex-col items-center text-center gap-2">
          ...
          <p className="text-sm font-medium text-[var(--c-textSecondary)]">Create an account to report civic issues</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4" noValidate>
          ...
          {/* Phone Number - Now on its own line */}
          <div className="flex flex-col gap-1">
            <label className="text-[10px] font-bold text-[var(--c-charcoal)] uppercase tracking-wider">Phone Number</label>
            <div className="relative">
              <Phone className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
              <input type="text" name="number" value={formData.number} onChange={handleChange} placeholder="9876543210" maxLength={10} className={inputClass} />
            </div>
            {errors.number && <p className="text-red-500 text-[10px]">{errors.number}</p>}
          </div>

          {/* Passwords - Stack on mobile, side-by-side on desktop */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            {/* Confirm Password */}
            <div className="flex flex-col gap-1">
              <label className="text-[10px] font-bold text-[var(--c-charcoal)] uppercase tracking-wider">Confirm</label>
              <div className="relative">
                <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
          ...
          <button type="submit" disabled={submitting} className="w-full mt-2 flex items-center justify-center gap-2 bg-[var(--c-olive)] hover:bg-[var(--c-oliveDark)] text-white font-bold text-sm py-3 rounded-xl transition-all shadow-md hover:shadow-lg transform hover:-translate-y-0.5">
            {submitting ? <Loader variant="inline" text="Creating..." /> : <>Create Account <ArrowRight className="w-4 h-4" /></>}
          </button>
        </form>
        ...
        <div className="flex flex-col gap-4 text-center mt-2">
          <p className="text-sm text-gray-500">
            Already have an account? <Link to="/login" className="text-[var(--c-olive)] font-black hover:underline">Sign in instead</Link>
          </p>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-2 sm:gap-4">
            <Link to="/authority/register" className="text-xs font-bold px-4 py-2 rounded-lg bg-[var(--c-accentGold)]/10 text-[var(--c-accentGold)] hover:bg-[var(--c-accentGold)]/20 transition-colors w-full sm:w-auto">
              Authority Registration
            </Link>
            <Link to="/authority/login" className="text-xs font-bold px-4 py-2 rounded-lg bg-[var(--c-sage)]/20 text-[var(--c-oliveDark)] hover:bg-[var(--c-sage)]/40 transition-colors w-full sm:w-auto">
              Authority Login
            </Link>
          </div>
        </div>
