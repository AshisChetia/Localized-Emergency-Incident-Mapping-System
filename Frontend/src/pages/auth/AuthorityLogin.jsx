...
const AuthorityLogin = () => {
  ...
  const handleChange = (e) => {
    ...
    if (pendingError) setPendingError(""); // Clear pending warning on type
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      login(token, user);
      toast.success(`Welcome to the Authority Portal, ${user.name}!`);
  ...
  return (
    <div style={authStyle} className="min-h-[calc(100vh-80px)] flex items-center justify-center p-4 relative overflow-hidden bg-[var(--c-offWhite)]">
      {/* Animated Blobs */}
      <div className="absolute top-10 left-10 w-72 h-72 bg-[var(--c-accentGold)] rounded-full mix-blend-multiply filter blur-3xl opacity-30 animate-blob"></div>
      <div className="absolute bottom-10 right-10 w-72 h-72 bg-[var(--c-sage)] rounded-full mix-blend-multiply filter blur-3xl opacity-50 animate-blob animation-delay-2000"></div>
      ...
      <div className="relative z-10 w-full max-w-md bg-white/80 backdrop-blur-xl border border-[var(--c-borderLight)] rounded-3xl shadow-2xl p-8 flex flex-col gap-6">
        ...
        <div className="flex flex-col items-center text-center gap-2">
          <div className="w-12 h-12 bg-[var(--c-charcoal)] rounded-2xl flex items-center justify-center shadow-md mb-2">
            <Building2 className="w-6 h-6 text-[var(--c-accentGold)]" />
          </div>
          <h1 className="text-2xl font-black text-[var(--c-charcoal)] tracking-tight" style={{ fontFamily: fonts.heading }}>
            Authority Portal
          </h1>
          <p className="text-sm text-gray-500">Sign in to your municipal dashboard</p>
        </div>

        {/* Pending Approval Warning */}
        {pendingError && (
          <div className="bg-[#FFF8E6] border border-[#F2DCA2] rounded-xl p-4 flex gap-3 text-left">
            <AlertCircle className="w-5 h-5 text-[var(--c-accentGold)] shrink-0 mt-0.5" />
            <div>
              <h3 className="text-sm font-bold text-[var(--c-charcoal)]">Approval Pending</h3>
              <p className="text-xs text-gray-600 mt-1 leading-relaxed">{pendingError}</p>
            </div>
          </div>
        )}
